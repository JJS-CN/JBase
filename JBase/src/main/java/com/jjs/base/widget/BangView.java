...
public class BangView extends View {
    ...
    private OnBangListener mBangListener;
    private int centerY;
    private int centerX;
    ...
    public BangView(Activity context) {
        super(context);
        init(context, null, 0);
    }

    public BangView(Activity context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs, 0);
    }

    public BangView(Activity context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs, defStyleAttr);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public BangView(Activity context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);
        init(context, attrs, defStyleAttr);
    }

    private void init(Activity activity, AttributeSet attrs, int defStyleAttr) {
        circlePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        circlePaint.setStyle(Paint.Style.FILL);
        circlePaint.setColor(Color.BLACK);
        ViewGroup rootView = (ViewGroup) activity.findViewById(Window.ID_ANDROID_CONTENT);
        BangView smallBang = new BangView(activity);
     ...
     * @param listener
     */
    public BangView setOnBangListener(OnBangListener listener) {
        this.mBangListener = listener;
        return this;
    }
     ...
     * @param newColors
     */
    public BangView setColors(int... newColors) {
     ...
     * @param dotNumber
     */
    public BangView setDotNumber(int dotNumber) {
    ...
    public void bang(final View view, OnBangListener listener) {
        bang(view, -1, listener);
    }

    //这里的参数二、可能会出现大小屏幕适配问题
    ...
    private void bang() {
        ...
        valueAnimator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                if (mBangListener != null) {
                    mBangListener.onAnimationEnd();
                }
            }
        });
        initDots();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (progress >= 0 && progress <= P1) {
            ...
            int endColor = colors.length == 1 ? colors[0] : colors[1];
            circlePaint.setStyle(Paint.Style.FILL);
     ...
     * 爆炸动画监听
     */
    public interface OnBangListener {
